...
import { AiGradingResult } from "../celpip";
...
export default function CelpipResultDialog({
  isOpen,
  onClose,
  title = "Test Results",
  standardResults,
  aiResult,
}: CelpipResultDialogProps) {
              ...
              {standardResults.length === 0 ? (
                ...
                standardResults.map((res, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`p-4 rounded-xl border ${isCorrect ? "bg-green-50 border-green-100 dark:bg-green-900/10 dark:border-green-800" : "bg-red-50 border-red-100 dark:bg-red-900/10 dark:border-red-800"}`}
                    >
                      {res.title && (
                      ...
                      <p className="font-medium text-gray-800 dark:text-white mb-3 text-sm">
                        {res.question}
                      </p>

                      <div className="flex flex-col gap-2 text-sm">
                        <div
                          className={`flex items-center gap-2 ${isCorrect ? "text-green-700 font-bold" : "text-red-600 font-medium"}`}
                        >
                          {isCorrect ? (
    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-300">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 justify-items-center">
        ...
        <div className="col-span-2 sm:col-span-1 flex justify-center w-full">
          <CircularProgress
            label="Overall"
            value={Number(result.overal_score) || 0}
            color="text-blue-600"
            size="lg"
          />
        </div>
        ...
        <CircularProgress
          label="Coherence"
          value={Number(result.coherance_score) || 0}
          color="text-indigo-600"
        />
        <CircularProgress
          label="Vocabulary"
          value={Number(result.vocabulary_score) || 0}
          color="text-purple-600"
        />
        <CircularProgress
          label="Readability"
          value={Number(result.readability_score) || 0}
          color="text-pink-600"
        />
        <CircularProgress
          label="Fulfillment"
          value={Number(result.fullfilment_score) || 0}
        ...
        {showFull && (
          <div className="mt-4 space-y-4 animate-in slide-in-from-top-2">
            ...
            <FeedbackSection
              ...
              content={result.coherance_issue}
              variant="issue"
            />
            <FeedbackSection
              title="Coherence Advice"
              content={result.coherance_advice}
              variant="advice"
            />
            <FeedbackSection
              title="Coherence Improve"
              content={result.coherance_improve}
              variant="improve"
            />
            ...
            <FeedbackSection
              ...
              content={result.vocabulary_issue}
              variant="issue"
            />
            <FeedbackSection
              title="Vocabulary Advice"
              content={result.vocabulary_advice}
              variant="advice"
            />
            <FeedbackSection
              title="Vocabulary Improve"
              content={result.vocabulary_improve}
              variant="improve"
            />
            ...
            <FeedbackSection
              ...
              content={result.readability_issue}
              variant="issue"
            />
            <FeedbackSection
              title="Readability Advice"
              content={result.readability_advice}
              variant="advice"
            />
            <FeedbackSection
              title="Readability Improve"
              content={result.readability_improve}
              variant="improve"
            />
            ...
            <FeedbackSection
              ...
              content={result.fullfilment_issue}
              variant="issue"
            />
            <FeedbackSection
              title="Fulfillment Advice"
              content={result.fullfilment_advice}
              variant="advice"
            />
            <FeedbackSection
              title="Fulfillment Improve"
              content={result.fullfilment_improve}
              variant="improve"
            />
            ...
            <FeedbackSection
              ...
              content={result["Score Improvement Tips"]}
              variant="advice"
            />
          </div>
        )}
    ...
    <div className="flex flex-col items-center gap-2">
      <div className={`relative flex items-center justify-center ${sizeClass}`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span
            className={`font-black text-gray-800 dark:text-white ${textClass}`}
          >
            {safeValue}
          </span>
        </div>
      </div>
      <span className="text-xs font-bold uppercase text-gray-500 tracking-wider text-center">
        {label}
      </span>
    </div>
  );
...
function FeedbackSection({
  ...
  const styles: Record<FeedbackVariant, string> = {
    issue:
      "bg-red-50 text-red-900 border-red-200 dark:bg-red-900/20 dark:border-red-800 dark:text-red-100",
    improve:
      "bg-emerald-50 text-emerald-900 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-100",
    advice:
      "bg-blue-50 text-blue-900 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-100",
  };

  return (
    <div
      className={`p-4 rounded-xl border ${styles[variant]} shadow-sm transition-colors`}
    >
      ...
      <p
        className="text-sm leading-relaxed font-medium text-justify"
        dir="auto"
      >
        {content}
      </p>
