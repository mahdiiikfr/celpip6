...
export default function CelpipPlayerView({
  src,
  transcript,
  autoPlay = false,
  className = "",
}: CelpipPlayerViewProps) {
          ...
          if (videoRef.current) {
            ...
            videoRef.current.play().catch((e2) => {
              if (e2.name !== "AbortError")
                console.error("Autoplay muted failed too", e2);
            });
          }
        });
      }
  ...
  if (isAudio) {
    return (
      <div
        className={`bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm relative group ${className}`}
      >
        {/* Audio Element */}
        <audio
          ref={videoRef as any}
  ...
  return (
    <div
      className={`bg-black rounded-2xl overflow-hidden shadow-sm border border-gray-800 relative group aspect-video ${className}`}
    >
      <video
        ref={videoRef}
        src={src}
      ...
      {/* Controls Overlay */}
      <div
        className={`absolute inset-0 bg-black/40 flex flex-col justify-end p-4 transition-opacity duration-300 ${isPlaying ? "opacity-0 group-hover:opacity-100" : "opacity-100"}`}
      >
        {/* Progress Bar */}
      ...
      {showTranscript && transcript && (
        <div className="absolute inset-0 bg-black/90 p-6 overflow-y-auto z-20 animate-in fade-in">
          ...
          <h4 className="text-sm font-bold text-amber-500 uppercase mb-4 sticky top-0 bg-black/90 py-2">
            Transcript
          </h4>
