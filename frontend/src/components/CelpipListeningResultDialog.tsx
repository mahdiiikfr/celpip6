...
interface CelpipListeningResultDialogProps {
  isOpen: boolean;
  onClose: () => void;
  score: number;
...
export default function CelpipListeningResultDialog({
  isOpen,
  onClose,
  score,
  totalQuestions,
  results,
}: CelpipListeningResultDialogProps) {
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="bg-white dark:bg-gray-900 flex flex-col rounded-t-[10px] h-[90vh] mt-24 fixed bottom-0 left-0 right-0 z-50 outline-none">
          ...
          <VisuallyHidden>
            ...
            <Drawer.Description>
              {t(
                "celpip.result_description",
                "Your listening test results and score.",
              )}
            </Drawer.Description>
          </VisuallyHidden>
          ...
          <div className="p-4 bg-white dark:bg-gray-900 rounded-t-[10px] flex-shrink-0 border-b border-gray-200 dark:border-gray-800">
            ...
            <div className="bg-amber-100 dark:bg-amber-900/30 p-4 rounded-xl flex flex-col items-center justify-center mt-2">
              ...
              <div className="text-4xl font-bold text-amber-600 dark:text-amber-400 mt-1">
                {score}{" "}
                <span className="text-xl text-amber-500/60 dark:text-amber-400/60">
                  / {totalQuestions}
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {results.map((item, index) => (
              <div
                key={index}
                className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm"
              >
